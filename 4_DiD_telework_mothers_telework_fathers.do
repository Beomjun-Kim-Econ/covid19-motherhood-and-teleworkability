*******************************************************
* 4_DiD_telework_mothers_telework_fathers.do
*
* 주 분석
*
...
capture mkdir "output/S_F4_DiD_telework_mothers_telework_fathers"
...
preserve
    ...
    save "output/S_F4_DiD_telework_mothers_telework_fathers/S_F4_employed_2020_2022.dta", replace
restore

merge m:1 pid using ///
    "output/S_F4_DiD_telework_mothers_telework_fathers/S_F4_employed_2020_2022.dta", ///
    keep(3) nogen
...
preserve
    ...
    save "output/S_F4_DiD_telework_mothers_telework_fathers/S_F4_baseline_2019.dta", replace
restore

merge m:1 pid using "output/S_F4_DiD_telework_mothers_telework_fathers/S_F4_baseline_2019.dta", ///
    keep(3) nogen
...
capture log close
log using "output/S_F4_DiD_telework_mothers_telework_fathers/S_F4_01_main_tests.log", text replace
...
reghdfe ln_weekly_income i.mother2019##ib2019.survey_year, ///
    absorb(pid survey_year occ2019#survey_year ind2019#survey_year ///
        region2019#survey_year) vce(cluster pid)
estimates store S_F4_wk_ev_u

reghdfe ln_weekly_income i.mother2019##ib2019.survey_year ///
    [pw=person_wgt2019], ///
    absorb(pid survey_year occ2019#survey_year ind2019#survey_year ///
        region2019#survey_year) vce(cluster pid)
estimates store S_F4_wk_ev_w

* 세 결과변수의 event study를 하나의 표에 저장한다.
esttab S_F4_hw_ev_u S_F4_hw_ev_w ///
    S_F4_hr_ev_u S_F4_hr_ev_w ///
    S_F4_wk_ev_u S_F4_wk_ev_w ///
    using "output/S_F4_DiD_telework_mothers_telework_fathers/S_F4_02_event_study.rtf", ///
    ...
    mtitles("시간당 임금 비가중" "시간당 임금 가중" ///
        "근로시간 비가중" "근로시간 가중" ///
        "주간소득 비가중" "주간소득 가중") ///
    title("재택가능 어머니-아버지 event study")

coefplot ///
    (S_F4_hw_ev_u, label("비가중")) ///
    (S_F4_hw_ev_w, label("가중")), ///
    ...
    title("재택가능 어머니-아버지: 시간당 임금 event study") ///
    note("기준연도: 2019년") ///
    legend(rows(1))

graph export "output/S_F4_DiD_telework_mothers_telework_fathers/S_F4_03_hourly_wage_event_study.png", ///
    width(2000) replace

coefplot ///
    (S_F4_hr_ev_u, label("비가중")) ///
    (S_F4_hr_ev_w, label("가중")), ///
    keep(1.mother2019#*.survey_year) vertical ///
    yline(0, lcolor(gs8)) xline(3.5, lpattern(dash) lcolor(gs8)) ///
    coeflabels( ///
        1.mother2019#2016.survey_year = "2016" ///
    ...
    title("재택가능 어머니-아버지: 근로시간 event study") ///
    note("기준연도: 2019년") ///
    legend(rows(1))

graph export "output/S_F4_DiD_telework_mothers_telework_fathers/S_F4_04_hours_event_study.png", ///
    width(2000) replace

coefplot ///
    (S_F4_wk_ev_u, label("비가중")) ///
    (S_F4_wk_ev_w, label("가중")), ///
    keep(1.mother2019#*.survey_year) vertical ///
    yline(0, lcolor(gs8)) xline(3.5, lpattern(dash) lcolor(gs8)) ///
    coeflabels( ///
        1.mother2019#2016.survey_year = "2016" ///
    ...
    title("재택가능 어머니-아버지: 주간 근로소득 event study") ///
    note("기준연도: 2019년") ///
    legend(rows(1))

graph export "output/S_F4_DiD_telework_mothers_telework_fathers/S_F4_05_weekly_income_event_study.png", ///
    width(2000) replace
...
reghdfe ln_hourly_wage i.mother2019##i.post2021_2022 ///
    ...
    absorb(pid survey_year occ2019#survey_year ind2019#survey_year ///
        region2019#survey_year) vce(cluster pid)
estimates store S_F4_hw_did_u

reghdfe ln_hourly_wage i.mother2019##i.post2021_2022 ///
    if !missing(post2021_2022) [pw=person_wgt2019], ///
    absorb(pid survey_year occ2019#survey_year ind2019#survey_year ///
        region2019#survey_year) vce(cluster pid)
estimates store S_F4_hw_did_w
...
reghdfe ln_hours i.mother2019##i.post2021_2022 ///
    ...
    absorb(pid survey_year occ2019#survey_year ind2019#survey_year ///
        region2019#survey_year) vce(cluster pid)
estimates store S_F4_hr_did_u

reghdfe ln_hours i.mother2019##i.post2021_2022 ///
    if !missing(post2021_2022) [pw=person_wgt2019], ///
    absorb(pid survey_year occ2019#survey_year ind2019#survey_year ///
        region2019#survey_year) vce(cluster pid)
estimates store S_F4_hr_did_w
...
reghdfe ln_weekly_income i.mother2019##i.post2021_2022 ///
    ...
    absorb(pid survey_year occ2019#survey_year ind2019#survey_year ///
        region2019#survey_year) vce(cluster pid)
estimates store S_F4_wk_did_u

reghdfe ln_weekly_income i.mother2019##i.post2021_2022 ///
    if !missing(post2021_2022) [pw=person_wgt2019], ///
    absorb(pid survey_year occ2019#survey_year ind2019#survey_year ///
        region2019#survey_year) vce(cluster pid)
estimates store S_F4_wk_did_w

esttab S_F4_hw_did_u S_F4_hw_did_w ///
    S_F4_hr_did_u S_F4_hr_did_w ///
    S_F4_wk_did_u S_F4_wk_did_w ///
    using "output/S_F4_DiD_telework_mothers_telework_fathers/S_F4_06_did.rtf", ///
    replace se star(* 0.10 ** 0.05 *** 0.01) ///
    ...
    mtitles("시간당 임금 비가중" "시간당 임금 가중" ///
        "근로시간 비가중" "근로시간 가중" ///
        "주간소득 비가중" "주간소득 가중") ///
    title("재택가능 어머니-아버지 평균 DiD")

coefplot ///
    (S_F4_hw_did_u, label("시간당 임금 비가중")) ///
    (S_F4_hw_did_w, label("시간당 임금 가중")) ///
    (S_F4_hr_did_u, label("근로시간 비가중")) ///
    (S_F4_hr_did_w, label("근로시간 가중")) ///
    (S_F4_wk_did_u, label("주간소득 비가중")) ///
    (S_F4_wk_did_w, label("주간소득 가중")), ///
    keep(1.mother2019#1.post2021_2022) ///
    coeflabels(1.mother2019#1.post2021_2022 = "어머니 × 사후기간") ///
    xline(0, lcolor(gs8)) ///
    title("재택가능 어머니-아버지 평균 DiD") ///
...
graph export "output/S_F4_DiD_telework_mothers_telework_fathers/S_F4_07_did_coefficients.png", ///
    width(2000) replace

log close
display as result "S_F4 세 결과변수 주 분석이 완성되었습니다."
